program Fingerprint
  ...
  parameter(nat=16,natx_sphere=100,ns=1,np=1,nconf=1)
  ...
  if (filetype.eq.'ascii') then
     open(unit=10,file='POSALL_test.ascii')
  else if (filetype.eq.'xyz') then
     open(unit=10,file='POSALL_test.xyz')
  else 
      stop 'unrecognized file format'
  endif

open(unit=20, file='data(const).dat')
  ...
  do itry = 1, 250
  	call fingerprint_eval(nat, natx_sphere, ns,np, alat, rxyz, rcov, fpall(1,1,iconf), const)
    write(20,*) const, fpall(1, 1,iconf), fpall(2, 1,iconf), fpall(3, 1,iconf), fpall(4, 1,iconf), fpall(5, 1,iconf), &
     fpall(6, 1, iconf), fpall(16, 1, iconf), fpall(19, 1, iconf),  fpall(20, 1, iconf), fpall(25, 1, iconf), &
     fpall(30, 1, iconf)
    const = const + 0.01
  enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine fingerprint_eval(nat, natx_sphere, ns,np, alat, rxyz, rcov, fp, const)
      implicit real*8 (a-h,o-z)
      parameter(nwork=100)
...
!       call xyz2eval(nat_sphere,rxyz_sphere,rcov_sphere,ns,np,amplitude,fp(1,lat))
       ...
       allocate(ovrlp(norb,norb),ovrla(norb,norb),eval(norb))
        ...
        call crtovrlp(nat_sphere,rxyz_sphere,alpha,cs,cp,ns,np,ovrlp, CN_symbs, const)
